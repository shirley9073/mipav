...
public class JDialogHoughHyperbolaChoice extends JDialogBase {
     ...
     * @param  maxR1
     * @param  count
     * @param  selectedHyperbola
     */
    public JDialogHoughHyperbolaChoice(Frame theParentFrame, float xCenter[], int xDimSource, float yCenter[], int yDimSource,
                                  float r1[], double minR1, double maxR1, float r2[], double minR2, double maxR2, 
                                  float theta[], short count[], boolean selectedHyperbola[]) {
        ...
        this.theta = theta;
        this.count = count;
        this.selectedHyperbola = selectedHyperbola;
        numHyperbolasFound = selectedHyperbola.length;
        init();
    }
        ...
        if (event.getSource() == OKButton) {
            for (i = 0; i < numHyperbolasFound; i++) {
                if (selectedArray[i].isSelected()) {
                    selectedHyperbola[i] = true;
                } // if (selectedArray[i].isSelected())
                else {
                    selectedHyperbola[i] = false;
                }
            } // for (i = 0; i < numHyperbolasFound; i++)
            okayPressed = true;
    ...
    private void init() {
        ...
        int i;
        JLabel xCenterArrayLabel[] = new JLabel[numHyperbolasFound];
        JLabel yCenterArrayLabel[] = new JLabel[numHyperbolasFound];
        JLabel r1ArrayLabel[] = new JLabel[numHyperbolasFound];
        JLabel r2ArrayLabel[] = new JLabel[numHyperbolasFound];
        JLabel thetaArrayLabel[] = new JLabel[numHyperbolasFound];
        JLabel countLabelArray[] = new JLabel[numHyperbolasFound];
        selectedArray = new JCheckBox[numHyperbolasFound];
        setTitle("Hough transform hyperbola selection");
  
        JPanel createPanel = new JPanel(new GridBagLayout());
        createPanel.setBorder(buildTitledBorder("Select hyperbola for display"));
        ...
        gbc.gridx = 6;
        JLabel selectLabel = new JLabel("Select hyperbolas");
