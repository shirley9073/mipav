...
public class JDialogEditSquareLength extends JDialogScriptableBase {
	...
	public JDialogEditSquareLength(Component parentComponent, String widthString, String measuredWidthString, String unitsString, float[] res, VOIBase voi, VOIManager voiManager) {
	...
	private void init() {
	  ...
	  gbc.gridwidth = 3;
      JLabel topLabel = new JLabel("Enter new Length : ");
	...
	private boolean editSquareLength() {
		...
		try {
			if(pixelRadio.isSelected()) {
				length = Float.valueOf(pixelField.getText().trim()).floatValue();
			}else {
				length = Float.valueOf(spaceField.getText().trim()).floatValue()/res[0];
		...
		voiManager.editSquareLength(voi, length);
		return true;
	}
