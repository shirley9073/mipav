 ...
 */
public class JDialogTopHat extends JDialogScriptableBase 
    ...
    private AlgorithmGrayScaleMorphology2D gsTopHatAlgo2D = null;
    
    private AlgorithmGrayScaleMorphology25D gsTopHatAlgo25D = null;
    
    private AlgorithmGrayScaleMorphology3D gsTopHatAlgo3D = null;
     ...
     */
    public JDialogTopHat(Frame theParentFrame, ModelImage im) {
    ...
    public void algorithmPerformed(AlgorithmBase algorithm) {
        ...
        if (algorithm instanceof AlgorithmGrayScaleMorphology2D) {
            ...
            if ((gsTopHatAlgo2D.isCompleted() == true) && (resultImage != null)) {
                ...
                try {
                    ...
                    // resultImage.setImageName("Top hat image");
            ...
            if ((gsTopHatAlgo25D.isCompleted() == true) && (resultImage != null)) {
            ...
            if ((gsTopHatAlgo3D.isCompleted() == true) && (resultImage != null)) {
                updateFileInfo(image, resultImage);
                resultImage.clearMask();
        ...
        if (gsTopHatAlgo2D != null) {
            gsTopHatAlgo2D.finalize();
            gsTopHatAlgo2D = null;
        }

        if (gsTopHatAlgo25D != null) {
            gsTopHatAlgo25D.finalize();
            gsTopHatAlgo25D = null;
        }

        if (gsTopHatAlgo3D != null) {
            gsTopHatAlgo3D.finalize();
            gsTopHatAlgo3D = null;
        }
    ...
    protected void callAlgorithm() {
        String name = makeImageName(image.getImageName(), "_top_hat");
            ...
            if (image.getNDims() == 2) { // source image is 2D
                ...
                if (outputPanel.isOutputNewImageSet()) {
                    ...
                    try {
                        ...
                        // Make algorithm
                        gsTopHatAlgo2D = new AlgorithmGrayScaleMorphology2D(resultImage, kernel, kernelSize,
                                                                AlgorithmGrayScaleMorphology2D.TOP_HAT, itersD, itersE, 0, 0,
                                                                outputPanel.isProcessWholeImageSet());
    
                        if (outputPanel.isProcessWholeImageSet() == false) {
                            gsTopHatAlgo2D.setMask(image.generateVOIMask());
                        }
    
                        // gsTopHatAlgo2D.setIterations(itersD, itersE);
                        // This is very important. Adding this object as a listener allows the algorithm to
                        // notify this object when it has completed or failed. See algorithm performed event.
                        // This is made possible by implementing AlgorithmedPerformed interface
                        gsTopHatAlgo2D.addListener(this);
    
                        createProgressBar(image.getImageName(), gsTopHatAlgo2D);
                        ...
                        if (isRunInSeparateThread()) {
                            ...
                            if (gsTopHatAlgo2D.startMethod(Thread.MIN_PRIORITY) == false) {
                        ...
                        MipavUtil.displayError("Dialog top hat: unable to allocate enough memory");
                    ...
                    try {
                        ...
                        gsTopHatAlgo2D = new AlgorithmGrayScaleMorphology2D(image, kernel, kernelSize, AlgorithmGrayScaleMorphology2D.TOP_HAT,
                ...
                if (outputPanel.isOutputNewImageSet()) {
                    ...
                    try {
                        ...
                        // Make algorithm
                        gsTopHatAlgo3D = new AlgorithmGrayScaleMorphology3D(resultImage, kernel, kernelSize,
                                                                AlgorithmGrayScaleMorphology3D.TOP_HAT, itersD, itersE, 0, 0,
                                                                outputPanel.isProcessWholeImageSet());
    
                        if (outputPanel.isProcessWholeImageSet() == false) {
                            gsTopHatAlgo3D.setMask(image.generateVOIMask());
                        }
    
                        // This is very important. Adding this object as a listener allows the algorithm to
                        // notify this object when it has completed or failed. See algorithm performed event.
                        // This is made possible by implementing AlgorithmedPerformed interface
                        gsTopHatAlgo3D.addListener(this);
    
                        createProgressBar(image.getImageName(), gsTopHatAlgo3D);
                        ...
                        if (isRunInSeparateThread()) {
                            ...
                            // Start the thread as a low priority because we wish to still have user interface work fast
                            if (gsTopHatAlgo3D.startMethod(Thread.MIN_PRIORITY) == false) {
                                MipavUtil.displayError("A thread is already running on this object");
                            }
                        } else {
                    ...
                    try {
                        ...
                        // Make algorithm
                        gsTopHatAlgo3D = new AlgorithmGrayScaleMorphology3D(image, kernel, kernelSize, AlgorithmGrayScaleMorphology3D.TOP_HAT,
                                                                itersD, itersE, 0, 0, outputPanel.isProcessWholeImageSet());
                ...
                try {
                    ...
                    // Make algorithm
                    gsTopHatAlgo25D = new AlgorithmGrayScaleMorphology25D(resultImage, kernel, kernelSize,
                                                              AlgorithmGrayScaleMorphology2D.TOP_HAT, itersD, itersE, 0, 0,
                                                              outputPanel.isProcessWholeImageSet());

                    if (outputPanel.isProcessWholeImageSet() == false) {
                        gsTopHatAlgo25D.setMask(image.generateVOIMask());
                    }

                    // gsTopHatAlgo25D.setIterations(itersD, itersE);
                    // This is very important. Adding this object as a listener allows the algorithm to
                    // notify this object when it has completed or failed. See algorithm performed event.
                    // This is made possible by implementing AlgorithmedPerformed interface
                    gsTopHatAlgo25D.addListener(this);

                    createProgressBar(image.getImageName(), gsTopHatAlgo25D);
                    ...
                    if (isRunInSeparateThread()) {
                        ...
                        // Start the thread as a low priority because we wish to still have user interface work fast.
                        if (gsTopHatAlgo25D.startMethod(Thread.MIN_PRIORITY) == false) {
                            MipavUtil.displayError("A thread is already running on this object");
                        }
                    } else {
                ...
                try {
                    ...
                    // No need to make new image space because the user has choosen to replace the source image
                    // Make the algorithm class
                    gsTopHatAlgo25D = new AlgorithmGrayScaleMorphology25D(image, kernel, kernelSize, AlgorithmGrayScaleMorphology25D.TOP_HAT,
                                                              itersD, itersE, 0, 0,
                                                              outputPanel.isProcessWholeImageSet());
    ...
    public ActionMetadata getActionMetadata() {
        return new MipavActionMetadata() {
            ...
            public String getDescription() {
                return new String("Subtracts the Open of the image from the image itself");
            }
            ...
            public String getShortLabel() {
                return new String("TopHat");
            }

            public String getLabel() {
                return new String("Top Hat");
            }
