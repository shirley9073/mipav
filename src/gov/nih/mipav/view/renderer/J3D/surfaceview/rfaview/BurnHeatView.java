...
/**
 * The default probe burning type view. The class define the thermal probe burning point's geometry shape, and probing
 * path mark behavior.
 *
 * @author  Ruida Cheng
 */
public class BurnHeatView extends BurnBaseView {
     ...
     */
    public BurnHeatView(SurfaceRender _surfaceRender, JPanelProbe _probePanel, BranchGroup _burnRootParentBG) {
    ...
    public void buildBurnSceneGraph(float radius, float time) {
        ...
        scale.set(scale.x, scale.y, scale.z * 2);
    ...
    public void startBurn(float tipLen, float time, int _index) {
        ...
        burnType = BurnBase.HEATBURN;
        burnIndex = _index;
        burnColor = orangeColor;

        Appearance app = new Appearance();
        Material mat = new Material(emissiveColor, emissiveColor, orange, sepcualarColor, 50.0f);
        ...
        generateEllipsoidMesh(BurnBase.HEATBURN, tipLen, 5, false);
        ...
        generateEllipsoidMesh(BurnBase.HEATBURN, tipLen, 5, true);
