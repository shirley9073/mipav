 ...
 * @see DecoderSpecs
 * */
public class DecoderRAWColor extends Decoder {//implements Runnable {
	...
	{ "nocolorspace",null,
	  "Ignore any pluginJP2.colorspace information in the image.","off"},
	{ "colorspace_debug", null,
	  "Print debugging messages when an error is encountered in the"+
	  " pluginJP2.colorspace module.","off"}
    };
     ...
     * display the image.
     * */
    public DecoderRAWColor(ParameterList pl, ImgScrollPane isp) {
	    super(pl,isp);
    	this.pl = pl;
     ...
     * values).
     * */
    public DecoderRAWColor(ParameterList pl) {
        this(pl,null);
    }
	...
	String btitle = "";
		...
		try {
		    ...
		    error("error processing jp2 pluginJP2.colorspace information"+
			  ((e.getMessage() != null) ?
			   (": "+e.getMessage()) : "    "),1,e);
		    return; }
	    } else { // Skip pluginJP2.colorspace mapping
                 color = ictransf; 
	    }
...
/*		
		...
		for(i=0; i<imwriter.length; i++) {
		    ...
		    } else if (outext.equalsIgnoreCase(".RAW")) {
		    	imwriter[i] = new ImgWriterRAW(out[i],decodedImage,i,
						     hd.isOriginalSigned(i));
...
*/			
	    	imwriter[0] = new ImgWriterRAW(out[0],decodedImage,nCompImg,false);
	    	imwriter[0].writeAll();
	    	imwriter[0].close();
	    }
...
// Try to decode only 1 single frame    
    public BlkImgDataSrc run1Slice(RandomAccessIO inBuff) {
        boolean verbose;
        int res; // resolution level to reconstruct
//        String infile;
        RandomAccessIO in;
...
//	String btitle = "";
        ...
        int nTiles = hi.siz.getNumTiles();
	    decSpec = hd.getDecoderSpecs();
//        JOptionPane.showMessageDialog(null, "num of comps:" + Integer.toString(nCompCod), "Debug", JOptionPane.INFORMATION_MESSAGE);        
            // Report information
            if(verbose) {
            ...
            depth = new int[nCompCod];
            for(i=0; i<nCompCod;i++) { depth[i] = hd.getOriginalBitDepth(i); }
//            System.out.println("Bit depth: " + Integer.toString(depth[0]));
            // **** Bit stream reader ****
            try {
	    ...
	    // **** Inverse component transformation **** 
            ...
            if(ff.JP2FFUsed && pl.getParameter("nocolorspace").equals("off")) {
            	try {
//            		JOptionPane.showMessageDialog(null, "hey, I am mapping color space", "Debug", JOptionPane.INFORMATION_MESSAGE);    			
            		csMap = new ColorSpace(in,hd,pl);
            		channels = hd.
            		createChannelDefinitionMapper(ictransf,csMap);
            		resampled  = hd.createResampler (channels,csMap);
            	...
            	color = ictransf; 
//            	JOptionPane.showMessageDialog(null, "Ouch, color space mapping is skipped!!!", "Debug", JOptionPane.INFORMATION_MESSAGE);
            }
...
//	    imwriter = new ImgWriter[1];
	    ...
	    if(verbose) {
	    	if(mrl!=res) {
		    FacilityManager.getMsgLogger().
			println("Reconstructing resolution "+res+" on "+mrl+" ("+breader.getImgWidth(res)+"x"+
				breader.getImgHeight(res)+")",8,8);
            }
            if(pl.getFloatParameter("rate")!=-1) {
               FacilityManager.getMsgLogger().println("Target rate = "+breader.getTargetRate()+
                                " bpp ("+ breader.getTargetNbytes()+" bytes)",8,8);
            }
	    }
		...
		// Print actually read bitrate
                ...
                if(pl.getIntParameter("ncb_quit") == -1) {
                  FacilityManager.getMsgLogger().println("Actual bitrate = "+bitrate+
                                " bpp (i.e. "+numBytes+" bytes)",8,8);
		} else {
            FacilityManager.getMsgLogger().println("Number of packet body bytes read = "+numBytes,
                                8,8);
		}
		FacilityManager.getMsgLogger().flush();
	    }
	    
	    //Return decoded Image and quit!
	    return decodedImage;
    ...
    private void printVersionAndCopyright() {
        FacilityManager.getMsgLogger()
            .println("JJ2000's JPEG 2000 DecoderRAW\n",2,4);
        FacilityManager.getMsgLogger()
