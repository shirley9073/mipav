 ...
 * @see EncoderSpecs
 * */
public class EncoderRAWColor implements Runnable {
     ...
     * values);
     * */
    public EncoderRAWColor(ParameterList pl) {
	this.pl = pl;
	defpl = pl.getDefaultParameterList();
    }

    public EncoderRAWColor(ParameterList pl, ModelImage image) {
    	this.pl = pl;
    	defpl = pl.getDefaultParameterList();
    	this.image = image;
    }
	...
	*/
     ...
     * @see #getExitCode
     * */
    public ByteArrayOutputStream run1Slice(ImgReaderRAWColorSlice img) {
        boolean verbose;
        boolean useFileFormat = false;
        boolean pphTile = false;
        boolean pphMain = false;
...
//		 JOptionPane.showMessageDialog(null, "get here:ImgReaderRAWColorSlice" + Integer.toString(imgType), "Debug", JOptionPane.INFORMATION_MESSAGE);		
	...
	//			 JOptionPane.showMessageDialog(null, s, "File pointer position", JOptionPane.INFORMATION_MESSAGE);			 
				 f.write(buff.toByteArray(),0,buff.size());
				 //if (sliceIdx==startSlice+14){
					// BEBufferedRandomAccessFile fSomeSlice = 
					//	 new BEBufferedRandomAccessFile("C:\\temp\\firstSlice.jp2",
	                  //        "rw",(int) (100 * 1024));
					 //fSomeSlice.write(buff.toByteArray(),0,buff.size());
					// fSomeSlice.close();
				 //}
			 }
		 }
...
//		 		 JOptionPane.showMessageDialog(null, "It's weird here", "Info", JOptionPane.INFORMATION_MESSAGE);
		 ImgReaderRAWColorSlice slice = new ImgReaderRAWColorSlice(pl.getParameter("i"),startSlice);		 
		 for(int sliceIdx=startSlice;sliceIdx<=endSlice;sliceIdx++) {
