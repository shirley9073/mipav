 ...
 */
public class AlgorithmRegELSUNCOAR35D extends AlgorithmBase {
     ...
     * @param  searchAlgorithm ELSUNC, NL2SOL, or LEVENBERG_MARQUARDT
     */
    public AlgorithmRegELSUNCOAR35D(ModelImage _image, int _costChoice, int _DOF, int _interp, int _interp2, int mode,
     ...
     * @param  searchAlgorithm ELSUNC, NL2SOL, or LEVENBERG_MARQUARDT
     */
    public AlgorithmRegELSUNCOAR35D(ModelImage _image, ModelImage _inputWeight, int _costChoice, int _DOF, int _interp,
                              ...
                              boolean fastMode, int _baseNumIter, int _numMinima, int searchAlgorithm) {
    ...
    private Vector<MatrixListItem>[] levelEight(ModelSimpleImage ref, ModelSimpleImage input) {
        ...
        AlgorithmELSUNCOpt3D elsunc = null;
        maxIter = baseNumIter * 4;
        int nDims = 3;
        if (DOF > 6) {
            nDims = 4;
        }
        elsunc = new AlgorithmELSUNCOpt3D(this, cog, nDims, cost, getTolerance(nDims), maxIter, searchAlgorithm);
        ...
        for (int i = 0; i < coarseNum; i++) {
            for (int j = 0; j < coarseNum; j++) {
                for (int k = 0; k < coarseNum; k++) {
                    preTransforms[i][j][k] = elsunc.getPoint(index++);
                    if (DOF > 6) {
        ...
        for (int i = 0; i < fineNum ; i++) {
            for (int j = 0; j < fineNum; j++) {
                for (int k = 0; k < fineNum; k++) {
                    ...
                    if (matrixList[i][j][k].cost < threshold) {
                        matrixList[i][j][k] = new MatrixListItem(elsunc.getCost(index),
                        		elsunc.getMatrix(index), elsunc.getPoint(index));
                        index++;
                    }
                }
            }
    ...
    private MatrixListItem levelOne(ModelSimpleImage ref, ModelSimpleImage input, MatrixListItem item, int maxIter) {
        ...
        AlgorithmELSUNCOpt3D elsunc = new AlgorithmELSUNCOpt3D(this, cog, degree, cost,
                getTolerance(degree), maxIter, searchAlgorithm);
