...
import gov.nih.mipav.model.algorithms.AlgorithmELSUNCOpt2D;
...
public class AlgorithmRegELSUNCOAR25D extends AlgorithmBase {
     ...
     * maxIter in the call to elsunc will be an integer multiple
     * of baseNumIter
     */
     ...
     * @param  _baseNumIter   Limits the number of iterations of elsunc algorithm. maxIter in the call to elsunc
     ...
     */
    public AlgorithmRegELSUNCOAR25D(ModelImage _image, int _costChoice, int _DOF, int _interp, int _interp2, boolean mode,
                               ...
                               int _baseNumIter, int _numMinima, int searchAlgorithm) {
        super(null, _image);
        inputImage = _image;
     ...
     * @param searchAlgorithm ESLUNC, LEVENBERG_MARQUARDT, or NL2SOL.
     */
    public AlgorithmRegELSUNCOAR25D(ModelImage _image, ModelImage _inputWeight, int _costChoice, int _DOF, int _interp,
                               ...
                               boolean transformVOIs, int _baseNumIter, int _numMinima, int searchAlgorithm) {
    ...
    public void runAlgorithm() {
        ...
        for (int m = 0; m < endIndex; m++) {
            ...
            if ((iNumber == (inputImage.getExtents()[2] - 1)) && regToAdjImage) {
                ...
                transform = new AlgorithmTransform(input1, tMat, interp2, resIso[0], resIso[1], extentsIso[0],
    ...
    private Vector<MatrixListItem>[] levelEight(ModelSimpleImage ref, ModelSimpleImage input) {
        ...
        elsunc = new AlgorithmELSUNCOpt2D(this, cog, nDims, cost, getTolerance(nDims), maxIter, false, searchAlgorithm);
        ...
        for(int i = 0; i < coarseNum; i++){
            transforms[i] = elsunc.getPoint(i);
        }
        ...
        for(int i = 0; i < fineNum; i++){
            if (matrixList[i].cost < threshold) {
                matrixList[i] = new MatrixListItem(elsunc.getCost(i), elsunc.getMatrix(i), elsunc.getPoint(i));
            }
        }
    ...
    private Vector<MatrixListItem> levelFour(ModelSimpleImage ref, ModelSimpleImage input, 
        ...
        for (Enumeration<MatrixListItem> en = minima.elements(); en.hasMoreElements() && !threadStopped;) {
            item.cost = elsunc.measureCost(en.nextElement().initial);
        }
    ...
    private MatrixListItem levelOne(ModelSimpleImage ref, ModelSimpleImage input, MatrixListItem item, int frame) {
        ...
        AlgorithmELSUNCOpt2D elsunc = new AlgorithmELSUNCOpt2D(this, cog, degree, cost,
                getTolerance(degree), maxIter, rigidFlag, searchAlgorithm);
    ...
    private MatrixListItem levelTwo(ModelSimpleImage ref, ModelSimpleImage input, Vector<MatrixListItem> minima) {
        ...
        AlgorithmELSUNCOpt2D elsunc = new AlgorithmELSUNCOpt2D(this, cog, degree, cost, getTolerance(degree), maxIter,
