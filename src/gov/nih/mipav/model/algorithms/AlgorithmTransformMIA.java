 ...
 * to be cancelled, we should NOT use the static method, but rather construct a new AlgorithmTransformMIA and run the
 ...
 * 
 * <p>
 * This version of AlgorithmTransformMIA includes a flag (passed as the last parameter) indicating whether to pad the image
 * volume.
 * </p>
 * 
 ...
 * @author Zohara Cohen
 */
public class AlgorithmTransformMIA extends AlgorithmBase {
     ...
     */
    public AlgorithmTransformMIA(final ModelImage srcImage, final TransMatrix xfrm, final int interp, final float oXres,
    ...
    /**
     * Creates a new AlgorithmTransformMIA object.
     * 
     * @param srcImage ModelImage to be transformed
     * @param xfrm Transformation matrix to be applied
     * @param interp Type of interpolation (NEAREST_NEIGHBOR, BILINEAR, BSPLINE3, BSPLINE4, etc)
    ...
    public AlgorithmTransformMIA(final ModelImage srcImage, final TransMatrix xfrm, final int interp, final float oXres,
        ...
        AlgorithmTransformMIA.imgOrient = srcImage.getFileInfo(0).getImageOrientation();

        AlgorithmTransformMIA.axisOrient = new int[srcImage.getFileInfo(0).getAxisOrientation().length];

        for (int i = 0; i < AlgorithmTransformMIA.axisOrient.length; i++) {
            AlgorithmTransformMIA.axisOrient[i] = srcImage.getFileInfo(0).getAxisOrientation()[i];
        }

        AlgorithmTransformMIA.imgOrigin = srcImage.getFileInfo(0).getOrigin().clone();

        final int type = srcImage.getType();

        /* Read the direction vector from the MipavCoordinateSystems class: */
        AlgorithmTransformMIA.direct = MipavCoordinateSystems.getModelDirections(srcImage);
        DIM = srcImage.getNDims();

        if (pad) {

            AlgorithmTransformMIA.margins = getImageMargins(srcImage, xfrm, oXres, oYres);
            Preferences.debug("Padding is " + AlgorithmTransformMIA.margins[0] + ", " + AlgorithmTransformMIA.margins[1]
                    + ".\n", Preferences.DEBUG_ALGORITHM);
            updateOriginMargins2D();

            // System.out.println("Image origin with padding: " +imgOrigin[0] +" " +imgOrigin[1]);
            oXdim = oXdim + AlgorithmTransformMIA.margins[0] + AlgorithmTransformMIA.margins[2];
            oYdim = oYdim + AlgorithmTransformMIA.margins[1] + AlgorithmTransformMIA.margins[3];
        } else {

            for (int m = 0; m < 4; m++) {
                AlgorithmTransformMIA.margins[m] = 0;
            }
        }
        ...
        if (srcImage.getNDims() == 2) {
            ...
            if ( ( (interp == AlgorithmTransformMIA.WSINC) || (interp == AlgorithmTransformMIA.CUBIC_LAGRANGIAN)
                    || (interp == AlgorithmTransformMIA.QUINTIC_LAGRANGIAN) || (interp == AlgorithmTransformMIA.HEPTIC_LAGRANGIAN))
                    && ( !clip) && ( (type == ModelStorageBase.BYTE) || (type == ModelStorageBase.UBYTE))) {
                destImage = new ModelImage(ModelStorageBase.SHORT, extents, name);
            } else if ( ( (interp == AlgorithmTransformMIA.WSINC) || (interp == AlgorithmTransformMIA.CUBIC_LAGRANGIAN)
                    || (interp == AlgorithmTransformMIA.QUINTIC_LAGRANGIAN) || (interp == AlgorithmTransformMIA.HEPTIC_LAGRANGIAN))
            ...
            oZdim = iZdim;
            AlgorithmTransformMIA.startPos = AlgorithmTransformMIA.imgOrigin[2]; // temporarily set origin for all slices as
            ...
            AlgorithmTransformMIA.startPos = AlgorithmTransformMIA.imgOrigin[2]; // temporarily set origin for all slices as
            // origin of first slice
            AlgorithmTransformMIA.startTime = AlgorithmTransformMIA.imgOrigin[3];
     ...
     * @param pad if <code>true</code> output image is padded so that none of the image is clipped
     */
    public AlgorithmTransformMIA(final ModelImage _srcImage, final TransMatrix xfrm, final int interp, final float _oXres,
    ...
    public AlgorithmTransformMIA(final ModelImage _srcImage, final TransMatrix xfrm, final int interp, final float _oXres,
        ...
        if (pad) {
            ...
            AlgorithmTransformMIA.margins = AlgorithmTransformMIA.getImageMargins(srcImage, xfrm, oXres, oYres, oZres);
            Preferences.debug("Padding is " + AlgorithmTransformMIA.margins[0] + ", " + AlgorithmTransformMIA.margins[1]
                    + " and " + AlgorithmTransformMIA.margins[2] + ".\n", Preferences.DEBUG_ALGORITHM);
            updateOriginMargins();

            // System.out.println("Image origin with padding: " +imgOrigin[0] +" " +imgOrigin[1] +" " +imgOrigin[2]);
            oXdim = oXdim + AlgorithmTransformMIA.margins[0] + AlgorithmTransformMIA.margins[3];
            oYdim = oYdim + AlgorithmTransformMIA.margins[1] + AlgorithmTransformMIA.margins[4];
            oZdim = oZdim + AlgorithmTransformMIA.margins[2] + AlgorithmTransformMIA.margins[5];
        } else {
        ...
        AlgorithmTransformMIA.startPos = AlgorithmTransformMIA.imgOrigin[2];
    ...
    public static ModelImage bspline(final ModelImage image, final ModelImage resultImage, final int degree,
        ...
        try {
            ...
            final TransMatrix kTM = AlgorithmTransformMIA.matrixtoInverseArray(xfrm);
            T00 = (double)kTM.M00;
            T01 = (double)kTM.M01;
            T02 = (double)kTM.M02;
            T03 = (double)kTM.M03;
    ...
    public static int[] getImageMargins(final ModelImage srcImage, final TransMatrix transMatrix, final float dxOut,
        ...
        AlgorithmTransformMIA.margins[0] = leftPad;
        AlgorithmTransformMIA.margins[1] = topPad;
        AlgorithmTransformMIA.margins[2] = front;
        AlgorithmTransformMIA.margins[3] = rightPad;
        AlgorithmTransformMIA.margins[4] = bottomPad;
        AlgorithmTransformMIA.margins[5] = back;

        return AlgorithmTransformMIA.margins;
    }
    ...
    public static final void transformBilinear(final ModelImage image, final ModelImage transformedImg,
        ...
        AlgorithmTransformMIA.transformBilinear(image, transformedImg, trans, progressBar, true);
    }
    ...
    public static final void transformBilinear(final ModelImage image, final ModelImage transformedImg,
        ...
        AlgorithmTransformMIA.imgOrigin = image.getFileInfo(0).getOrigin().clone();

        if (AlgorithmTransformMIA.updateOrigin) {
            AlgorithmTransformMIA.updateOrigin(trans);
        }
        ...
        AlgorithmTransformMIA.updateFileInfo(image, transformedImg, transformedImg.getFileInfo(0).getResolutions(), image
        ...
        final TransMatrix kTM = AlgorithmTransformMIA.matrixtoInverseArray(trans);
        T00 = kTM.M00;
        T01 = kTM.M01;
        T02 = kTM.M02;
        T10 = kTM.M10;
    ...
    public static final void transformBilinear(final float[] imgBuf, final ModelImage transformedImg,
        ...
        AlgorithmTransformMIA.transformBilinear(imgBuf, transformedImg, trans, iXdim, iYdim, iXres, iYres, progressBar,
                true);
    }
    ...
    public static final void transformBilinear(final float[] imgBuf, final ModelImage transformedImg,
        ...
        AlgorithmTransformMIA.imgOrigin = transformedImg.getFileInfo(0).getOrigin().clone();
    ...
    public static final void transformBilinear(final float[] imgBuf, final float[] tImgBuf, final TransMatrix trans,
        ...
        AlgorithmTransformMIA.transformBilinear(imgBuf, tImgBuf, trans, iXdim, iYdim, iXres, iYres, oXdim, oYdim, oXres,
    ...
    public static final void transformBilinearC(final ModelImage image, final ModelImage transformedImg,
        ...
        AlgorithmTransformMIA.updateFileInfo(image, transformedImg, resolutions, image.getFileInfo()[0]
        ...
        try {
            ...
            MipavUtil.displayError("AlgorithmTransformMIA: IOException Error on importData");
        }
    ...
    public static final void transformTrilinear(final ModelImage image, final ModelImage transformedImg,
            final TransMatrix trans, final ViewJProgressBar progressBar) {
        AlgorithmTransformMIA.transformTrilinear(image, transformedImg, trans, progressBar, true);
    }
    ...
    public static final void transformTrilinear(final float[] imgBuffer, final ModelImage transformedImg,
        ...
        AlgorithmTransformMIA.transformTrilinear(imgBuffer, transformedImg, trans, iXdim, iYdim, iZdim, iXres, iYres,
    ...
    public static final void transformTrilinear(final float[] imgBuffer, final float[] tImgBuf,
        ...
        AlgorithmTransformMIA.transformTrilinear(imgBuffer, tImgBuf, trans, iXdim, iYdim, iZdim, iXres, iYres, iZres,
    ...
    private int[] getImageMargins(final ModelImage srcImage, final TransMatrix transMatrix, final float dxOut,
        ...
        AlgorithmTransformMIA.margins[0] = leftPad;
        AlgorithmTransformMIA.margins[1] = topPad;
        AlgorithmTransformMIA.margins[2] = rightPad;
        AlgorithmTransformMIA.margins[3] = bottomPad;
    ...
    public void runAlgorithm() {
        ...
        if (AlgorithmTransformMIA.updateOrigin) {
            //AlgorithmTransformMIA.updateOrigin(this.transMatrix);
            updateOrigin();
        }
        ...
        AlgorithmTransformMIA.updateFileInfo(srcImage, destImage, destResolutions, oUnits, this.transMatrix, isSATransform, null);
        ...
        if (transMatrix.isIdentity()) {
            ...
            // the transformation matrix in AlgorithmTransformMIA.transform()
    ...
    public void setUpdateOriginFlag(final boolean originFlag) {
        AlgorithmTransformMIA.updateOrigin = originFlag;
    }
    ...
    private static void updateFileInfo(final ModelImage image, final ModelImage resultImage, final float[] resolutions,
        ...
        if (resultImage.getNDims() == 2) {
            ...
            if (useSATransform) {
                AlgorithmTransformMIA.imgOrient = FileInfoBase.AXIAL;
                AlgorithmTransformMIA.axisOrient[0] = FileInfoBase.ORI_R2L_TYPE;
                AlgorithmTransformMIA.axisOrient[1] = FileInfoBase.ORI_A2P_TYPE;
                AlgorithmTransformMIA.axisOrient[2] = FileInfoBase.ORI_I2S_TYPE;
            }
            ...
            for (int i = 0; i < resultImage.getExtents()[2]; i++) {
                ...
                fileInfo[i].setImageOrientation(AlgorithmTransformMIA.imgOrient);
                fileInfo[i].setAxisOrientation(AlgorithmTransformMIA.axisOrient);
                fileInfo[i].setUnitsOfMeasure(units);

                fileInfo[i].setOrigin(AlgorithmTransformMIA.imgOrigin);
                AlgorithmTransformMIA.imgOrigin[2] = AlgorithmTransformMIA.startPos
                        + (AlgorithmTransformMIA.direct[2] * i * resolutions[2]);
            ...
            for (int i = 0; i < (resultImage.getExtents()[2] * resultImage.getExtents()[3]); i++) {
                ...
                fileInfo[i].setAxisOrientation(AlgorithmTransformMIA.axisOrient);
                AlgorithmTransformMIA.imgOrigin[2] = AlgorithmTransformMIA.startPos
                        + (AlgorithmTransformMIA.direct[2] * (i % resultImage.getExtents()[2]) * resolutions[2]);
                AlgorithmTransformMIA.imgOrigin[3] = AlgorithmTransformMIA.startTime + (i / resultImage.getExtents()[2])
                        * resolutions[3];
                fileInfo[i].setOrigin(AlgorithmTransformMIA.imgOrigin);
    ...
    private void updateOrigin() {
        ...
        try {
            ...
            if (trans.getDim() == 3) {
                ...
                //System.err.println("Before: AlgorithmTransformMIA.imgOrigin[0] = " + AlgorithmTransformMIA.imgOrigin[0] +
                //		"AlgorithmTransformMIA.imgOrigin[1] = " + AlgorithmTransformMIA.imgOrigin[1] +
                //		"AlgorithmTransformMIA.imgOrigin[2] = " + AlgorithmTransformMIA.imgOrigin[2]);
                trans.transform(imgOrigin[0], imgOrigin[1], imgOrigin[2], imgOrigin);
                
                //System.err.println("After: AlgorithmTransformMIA.imgOrigin[0] = " + AlgorithmTransformMIA.imgOrigin[0] +
                //		"AlgorithmTransformMIA.imgOrigin[1] = " + AlgorithmTransformMIA.imgOrigin[1] +
                //		"AlgorithmTransformMIA.imgOrigin[2] = " + AlgorithmTransformMIA.imgOrigin[2]);
            }
    ...
    private void transform() {
        ...
        try {
            ...
            xfrm = AlgorithmTransformMIA.matrixtoInverseArray(trans);
            ...
            if ( (srcImage.getType() == ModelStorageBase.ARGB) && (interp == AlgorithmTransformMIA.TRILINEAR)) {
        ...
        if (bufferFactor == 1) { // black and white
            ...
            if ( (do25D) && (DIM == 4)) {
                ...
                if (interp == AlgorithmTransformMIA.NEAREST_NEIGHBOR) {
                    transformNearestNeighbor4Dim2D(imgBuf, xfrm);
                } else if (interp == AlgorithmTransformMIA.BILINEAR) {
                    transformBilinear4D(imgBuf, xfrm);
                } else if (interp == AlgorithmTransformMIA.BSPLINE3) {
                    transformAlgorithmBspline2D(imgBuf, xfrm, 3);
                } else if (interp == AlgorithmTransformMIA.BSPLINE4) {
                	transformAlgorithmBspline2D(imgBuf, xfrm, 4);
                } else if (interp == AlgorithmTransformMIA.CUBIC_LAGRANGIAN) {
                    transformCubicLagrangian4Dim2D(imgBuf, xfrm, clip);
                } else if (interp == AlgorithmTransformMIA.QUINTIC_LAGRANGIAN) {
                    transformQuinticLagrangian4Dim2D(imgBuf, xfrm, clip);
                } else if (interp == AlgorithmTransformMIA.HEPTIC_LAGRANGIAN) {
                    transformHepticLagrangian4Dim2D(imgBuf, xfrm, clip);
                } else if (interp == AlgorithmTransformMIA.WSINC) {
                ...
                if (interp == AlgorithmTransformMIA.BILINEAR) {
                ...
                } else if (interp == AlgorithmTransformMIA.TRILINEAR) {
        ...
        for (index = 0; index < voiVector.size(); index++) {
        	...
        	for (index2 = 0; index2 < index2Size; index2++) {
		        ...
		        for (i = 0; (i < oXdim) && !threadStopped; i++) {
		            ...
		            if (pad) {
		                iAdj = i - AlgorithmTransformMIA.margins[0];
		            } else {
		                iAdj = i;
		            }
		            ...
		            for (j = 0; (j < oYdim) && !threadStopped; j++) {
		                ...
		                // transform i,j
		                if (pad) {
		                    jAdj = j - AlgorithmTransformMIA.margins[1];
		                } else {
		                    jAdj = j;
		                }
        ...
        for (index = 0; index < voiVector.size(); index++) {
        	...
        	for (index2 = 0; index2 < index2Size; index2++) {
		        ...
		        for (k = 0; (k < oZdim) && !threadStopped; k++) {
		            ...
		            if (pad) {
		                kAdj = k - AlgorithmTransformMIA.margins[2];
		            } else {
		                kAdj = k;
		            }
    ...
    private void transformCubicLagrangian4DC(final double[] imgBuf, final float imgBuffer2[], final TransMatrix kTM,
        ...
        for (l = 0; (l < oTdim) && !threadStopped; l++) {
            ...
            try {
                ...
                MipavUtil.displayError("AlgorithmTransformMIA: IOException on destImage.importData");
            }
        ...
        for (k = 0; (k < oZdim) && !threadStopped; k++) {
            ...
            if (k < (oZdim - 1)) {
                ...
                try {
                    ...
                    MipavUtil.displayError("AlgorithmTransformMIA: IOException Error on exportData");
                    setCompleted(false);
    ...
    private void transformTrilinear(final double[] imgBuffer, final TransMatrix kTM) {
        ...
        try {
            ...
            displayError("AlgorithmTransformMIA: IOException on destImage.importdata(0,imgBUf2, false).");
    ...
    private void updateOriginMargins() {
        ...
        tx = AlgorithmTransformMIA.direct[0] * AlgorithmTransformMIA.margins[0] * oXres;
        ty = AlgorithmTransformMIA.direct[1] * AlgorithmTransformMIA.margins[1] * oYres;
        tz = AlgorithmTransformMIA.direct[2] * AlgorithmTransformMIA.margins[2] * oZres;

        // System.out.println("Image origin before padding: " +imgOrigin[0] +" " +imgOrigin[1] +" " +imgOrigin[2]);
        AlgorithmTransformMIA.imgOrigin[0] -= tx;
        AlgorithmTransformMIA.imgOrigin[1] -= ty;
        AlgorithmTransformMIA.imgOrigin[2] -= tz;
    }
