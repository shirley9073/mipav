...
public class CLFFTPlan {
	 ...
	 */
	public CLFFTPlan(cl_context context, int[] sizes, CLFFTDataFormat dataFormat, cl_device_id gpu) throws InvalidContextException {
		boolean isPow2 = true;
		...
		while (!done) {
			...
			this.program = clCreateProgramWithSource(context, 1, 
					new String[]{ kernel_string.toString() }, null, null);
	...
	public void release() {
		if (tempmemobj != null) {
			clReleaseMemObject(tempmemobj);
			tempmemobj = null;
		}
		if (tempmemobj_real != null) {
			clReleaseMemObject(tempmemobj_real);
			tempmemobj_real = null;
		}
		if (tempmemobj_imag != null) {
			clReleaseMemObject(tempmemobj_imag);
			tempmemobj_imag = null;
		}        
	...
	void allocateTemporaryBufferInterleaved(int batchSize) {
		if (temp_buffer_needed && last_batch_size != batchSize) {
			...
			tempmemobj = CL.clCreateBuffer(context,
					CL.CL_MEM_READ_WRITE,
					Sizeof.cl_float * tmpLength, null, null);
		}
	}
	 ...
	 */
	public void executeInterleaved(cl_command_queue queue, int batchSize, CLFFTDirection dir,
			cl_mem data_in, cl_mem data_out,
			cl_event condition, cl_event event) {
		int s;
		...
		if (temp_buffer_needed) {
			...
			for (CLFFTKernelInfo kernelInfo : kernel_list) {
				...
				clSetKernelArg( kernelInfo.kernel, 0, Sizeof.cl_mem, Pointer.to((memObj[currRead])));
				clSetKernelArg( kernelInfo.kernel, 1, Sizeof.cl_mem, Pointer.to((memObj[currWrite])));
				clSetKernelArg( kernelInfo.kernel, 2, Sizeof.cl_int, Pointer.to(new int[]{ dir.value() }));
				clSetKernelArg( kernelInfo.kernel, 3, Sizeof.cl_int, Pointer.to(new int[]{ wd.batchSize }));
				long[] globalWork = new long[]{wd.gWorkItems, 1};
				long[] localWork = new long[]{wd.lWorkItems, 1};
				clEnqueueNDRangeKernel(queue, kernelInfo.kernel, 2, null, 
	...
	void allocateTemporaryBufferPlanar(int batchSize) {
		if (temp_buffer_needed && last_batch_size != batchSize) {
			...
			tempmemobj_real = CL.clCreateBuffer(context,
					CL.CL_MEM_READ_WRITE,
					Sizeof.cl_float * tmpLength, null, null);
			tempmemobj_imag = CL.clCreateBuffer(context,
					CL.CL_MEM_READ_WRITE,
					Sizeof.cl_float * tmpLength, null, null);
		}
	}
	 ...
	 * @param event
	 */
	public void executePlanar(cl_command_queue queue, int batchSize, CLFFTDirection dir,
			cl_mem data_in_real,cl_mem data_in_imag, cl_mem data_out_real,cl_mem data_out_imag,
			cl_event contition, cl_event event) {
		int s;
		...
		cl_mem[] memObj_real = new cl_mem[3];
		cl_mem[] memObj_imag = new cl_mem[3];
		...
		if (temp_buffer_needed) {
			...
			for (CLFFTKernelInfo kernelInfo : kernel_list) {
				...
				clSetKernelArg( kernelInfo.kernel, 0, Sizeof.cl_mem, Pointer.to((memObj_real[currRead])));
				clSetKernelArg( kernelInfo.kernel, 1, Sizeof.cl_mem, Pointer.to((memObj_imag[currRead])));
				clSetKernelArg( kernelInfo.kernel, 2, Sizeof.cl_mem, Pointer.to((memObj_real[currWrite])));
				clSetKernelArg( kernelInfo.kernel, 3, Sizeof.cl_mem, Pointer.to((memObj_imag[currWrite])));
				clSetKernelArg( kernelInfo.kernel, 4, Sizeof.cl_int, Pointer.to(new int[]{ dir.value() }));
				clSetKernelArg( kernelInfo.kernel, 5, Sizeof.cl_int, Pointer.to(new int[]{ wd.batchSize }));
				long[] globalWork = new long[]{wd.gWorkItems};
				long[] localWork = new long[]{wd.lWorkItems};
				clEnqueueNDRangeKernel(queue, kernelInfo.kernel, 1, null, 
						globalWork, localWork, 0, null, null);
	...
	private void createKernelList() {
		...
		for (CLFFTKernelInfo kinfo : this.kernel_list) {
			kinfo.kernel = clCreateKernel(program, kinfo.kernel_name, null);
		}

		if (format == format.SplitComplexFormat) {
			twist_kernel = clCreateKernel(program, "clFFT_1DTwistSplit", null);
		} else {
			twist_kernel = clCreateKernel(program, "clFFT_1DTwistInterleaved", null);
		}
	}

	private boolean getPatchingRequired(cl_device_id devices) {
		int i;
		//for (i = 0; i < devices.length; i++) {
		for (CLFFTKernelInfo kInfo : kernel_list) {
			int numValues = 1;
			long[] workGroupSize = new long[numValues];
			clGetKernelWorkGroupInfo( kInfo.kernel, devices, CL.CL_KERNEL_WORK_GROUP_SIZE, Sizeof.cl_long * numValues, Pointer.to(workGroupSize), null);
			if ( workGroupSize[0]  < kInfo.num_workitems_per_workgroup) {
				return true;
			}
		}
		//}
	...
	long getMaxKernelWorkGroupSize(cl_device_id devices) {
