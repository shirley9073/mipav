 ...
 * @see  PlugInAlgorithm
 */
public class PlugInFociAndStrands implements PlugInAlgorithm {
    ...
    public void run(Frame parentFrame, ModelImage image) {
        ...
        if (parentFrame instanceof ViewJFrameImage) {
            new PlugInDialogFociAndStrands(parentFrame, image);
        } else {
            MipavUtil.displayError("PlugIn FociAndStrands only runs on an image frame.");
        }
    }
}
