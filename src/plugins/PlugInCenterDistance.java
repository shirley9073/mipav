 ...
 * @see  PlugInAlgorithm
 */
public class PlugInCenterDistance implements PlugInAlgorithm {
    ...
    public void run(Frame parentFrame, ModelImage image) {
        ...
        if (parentFrame instanceof ViewJFrameImage) {
            new PlugInDialogCenterDistance(parentFrame, image);
        } else {
            MipavUtil.displayError("PlugIn CenterDistance only runs on an image frame.");
        }
    }
}
