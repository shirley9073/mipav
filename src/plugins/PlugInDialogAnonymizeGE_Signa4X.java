...
/**
 * Visual Interface to anonymize GE_Signa4X images by the directory-full. Multiple directories may be chosen to operate on;
...
public class PlugInDialogAnonymizeGE_Signa4X extends JDialogBase {
     ...
     */
    public PlugInDialogAnonymizeGE_Signa4X () {
     ...
     // get the selected directory
        ...
        setTitle("Anonymize GE_Signa4X directory");
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (command.equals("New")) {
            ...
            if (randTopButton.isSelected()) {
                toplevelOnly = PlugInDialogAnonymizeGE_Signa4X.RANDOM_DIRECTORY_NAME;
            } else if (randChildButton.isSelected()) {
                toplevelOnly = PlugInDialogAnonymizeGE_Signa4X.RANDOM_SUBDIRECTORY_NAME;
            } else {
                toplevelOnly = PlugInDialogAnonymizeGE_Signa4X.NO_RANDOM_NAMES;
            }
    ...
    public class AnonymizeDicomDirectories extends Thread {
        ...
        /** DOCUMENT ME! */
        private PlugInDialogAnonymizeGE_Signa4X anonDialog; // for notification of completion
         ...
         */
        public void setFinishNotification(PlugInDialogAnonymizeGE_Signa4X jsa) {
            anonDialog = jsa;
        }
        ...
        private boolean anonymizeImage(File imageFile, File destinationDirectory, int[] randomSuffixList,
            ...
            try {
                ...
                //if (!suffix.equalsIgnoreCase(".sig")) {
                    if (FileUtility.isGESigna4X(imageFile.getName(), imageFile.getParent() + File.separator, true) != FileUtility.GE_SIGNA4X) {
                        writeToLog(imageFile.getAbsolutePath(), false);
                        writeToLog(" -- Not a GE_Signa4X file.  File skipped.", true);
                        return false;
                    }
                //}
                
                io.setFileDir(imageFile.getParent() + File.separator);
                mi = io.readGESigna4X(imageFile.getName(), imageFile.getParent() + File.separator);
                writeToLog(imageFile.getAbsolutePath(), false);
                ...
                if (mi != null) {
                    ...
                    return true;
                } else { // not a GE_Signa4X file.  leave it.
                    writeToLog(" -- Not a GE_Signa4X file.  File not Touched. Model Image = null.", true);
                ...
                writeToLog(imageFile.getAbsolutePath() + "  -- Not a GE_Signa4X file.  File not Touched.", true);
                mi = null;
