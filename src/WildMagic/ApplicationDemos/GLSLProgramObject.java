...
public class GLSLProgramObject
{
	...
	public void attachFragmentShader(GL2 gl, String filename)
	{
		...
		if ( fileURL != null )
		{
			...
			int iID = gl.glCreateShader( GL2ES2.GL_FRAGMENT_SHADER );
			...
			gl.glGetShaderiv( iID, GL2ES2.GL_COMPILE_STATUS, params, 0);
			if ( params[0] != 1 )
			{
				System.err.println(filename);
				System.err.println("compile status: " + params[0]);
				gl.glGetShaderiv( iID, GL2ES2.GL_INFO_LOG_LENGTH, params, 0);
	...
	public void attachVertexShader(GL2 gl, String filename)
	{
		...
		if ( fileURL != null )
		{
			...
			int iID = gl.glCreateShader( GL2ES2.GL_VERTEX_SHADER );
	...
	public void bindTexture2D(GL2 gl, String texname, int texid, int texunit) {
		bindTexture(gl, GL.GL_TEXTURE_2D, texname, texid, texunit);
	}

	public void bindTexture3D(GL2 gl, String texname, int texid, int texunit) {
		bindTexture(gl, GL2ES2.GL_TEXTURE_3D, texname, texid, texunit);
	}
	...
	public void link(GL2 gl)
	{
		...
		int[] params = new int[]{0};
		gl.glGetProgramiv( _progId, GL2ES2.GL_LINK_STATUS, params, 0);

		if (params[0] != 1) {

			System.err.println("link status: " + params[0]);
			gl.glGetProgramiv( _progId, GL2ES2.GL_INFO_LOG_LENGTH, params, 0);
			System.err.println("log length: " + params[0]);
